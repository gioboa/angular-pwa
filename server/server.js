...
app.post('/notify', function (req, res) {
    ...
    if(authSecret !== '') {
        let subscribersList = req.get('auth-secret') === AUTH_SECRET ? subscribers : subscribers.filter(subscriber => subscriber.name === authSecret);
        ...
        subscribers.forEach(subscriber => {
            ...
            webPush.sendNotification(subscriber.pushSubscription, payload, {}).then((response) =>{
                // console.log('OK');
...
app.post('/list', function (req, res) {
    ...
    if(authSecret === AUTH_SECRET) {
        subscribers.forEach((pushSubscription,index) => console.log(`${index} - ${pushSubscription.name}`));   
    }
...
app.post('/unsubscribe', function (req, res) {
    let subscription = req.body['subscription'];
    subscribers = subscribers.filter(subscriber => subscription['endpoint'] == subscriber.pushSubscription.endpoint);
